...
pixel_collider::pixel_collider(const std::shared_ptr<actor>& owner, const std::shared_ptr<sprite>& sprite)
	: collider(owner), m_sprite(sprite)
{
}
...
auto pixel_collider::collides_with(const collider& other, glm::vec2& impact_point) const -> bool
{
	...
	for (float y = y_start; y < y_end; ++y)
	{
		for (float x = x_start; x < x_end; ++x)
		{
			...
			const int ax = static_cast<int>(uv_a.x * tex_a_w);
			const int ay = static_cast<int>((1.0f - uv_a.y) * tex_a_h);
			const int bx = static_cast<int>(uv_b.x * tex_b_w);
			const int by = static_cast<int>((1.0f - uv_b.y) * tex_b_h);
